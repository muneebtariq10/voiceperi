...
export class PaymentPlanService {
  ...
  constructor(
    ...
    if (!process.env.STRIPE_SECRET_KEY) {
      console.warn(
        '⚠️ STRIPE_SECRET_KEY is not defined. Payment plans will not work.',
      );
    }
  ...
  async createCheckoutSession(
    ...
    const pricing = paymentPlan.pricings.find(
      (p) => p.type === (selectedPlan as typeof p.type),
    );
    ...
    const newPricings = await Promise.all(
      (updatePaymentPlanDto.pricing || []).map(async (priceOption) => {
        ...
        const hasChanged =
          ...
          Math.round(Number(existingMatch.price) * 100) !==
            Math.round(Number(priceOption.price) * 100) || // Price changed (accounting for float precision)
          existingMatch.discount_type !== priceOption.discount_type || // Discount type changed
          Number(existingMatch.discount ?? 0) !==
            Number(priceOption.discount ?? 0); // Discount value changed (handles null)

        if (existingMatch && hasChanged && existingMatch.stripe_price_id) {
          await this.stripe.prices.update(existingMatch.stripe_price_id, {
            active: false,
          });
        }
        ...
        if (hasChanged) {
          ...
          if (existingMatch) {
            ...
            existingMatch.discount_type =
              priceOption.discount_type || 'percentage';
  ...
  async remove(id: number): Promise<void> {
    ...
    for (const pricing of paymentPlan.pricings) {
      if (pricing.stripe_price_id) {
        try {
          await this.stripe.prices.update(pricing.stripe_price_id, {
            active: false,
          });
        } catch (err) {
          console.warn(
            `Stripe price ${pricing.stripe_price_id} could not be deactivated`,
            err.message,
          );
        }
      }
    ...
    if (paymentPlan.stripe_product_id) {
      try {
        await this.stripe.products.update(paymentPlan.stripe_product_id, {
          active: false,
        });
      } catch (err) {
        console.warn(
          `Stripe product ${paymentPlan.stripe_product_id} could not be archived`,
          err.message,
        );
      }
    }
