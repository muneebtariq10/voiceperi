...
export class ShopifyOrderAdapter implements IOrderProvider {
  ...
  async createOrder(payload: CreateOrderPayload): Promise<CreateOrderResult> {
    ...
    try {
      const { storeUrl, accessToken } = await this.getShopifyCredentials(
        false,
        payload.businessId,
      );
      ...
      try {
        ...
        if (err.message === 'UNAUTHORIZED') {
          this.logger.log(
            'Shopify access token is expired or invalid. Attempting to refresh...',
          );
          const creds = await this.getShopifyCredentials(
            true,
            payload.businessId,
          );
  ...
  async reorderPastOrder(
    ...
    return {
      success: false,
      message:
        'Reorder logic requiring fetching past line items is not fully implemented yet for Shopify.',
    };
  }
